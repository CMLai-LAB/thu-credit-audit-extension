...
export async function loadSubMajr() {
  setStatus('載入子學系清單…');
  const { ok, html, error } = await chrome.runtime.sendMessage({
    type: 'LOAD_SUBMAJR_OPTIONS',
    payload: { stype: els.stype().value, majr: els.majr().value }
  });
  if (!ok) { setStatus('載入失敗：' + error); return; }
...
export async function handleFetch() {
  ...
  const subMajr = els.subMajrOptions();

  const payload = subMajr && subMajr.value
    ? { setyear, stype, majr, subMajr: subMajr.value }
    : { setyear, stype, majr };
